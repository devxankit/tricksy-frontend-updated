...
import UserDriverTracker from '../components/UserDriverTracker';
...
export default function UserPanel() {
  ...
  useEffect(() => {
    ...
    if (userData) {
      ...
      setEditForm(JSON.parse(userData));
    }
    fetchAccommodations();
  }, []);
  ...
  const fetchAccommodations = async () => {
    try {
      const response = await api.get('/accommodation/user');
  ...
  const handleAccommodationSuccess = (newAccommodation) => {
        ...
        {mobileMenuOpen && (
          <div className="bg-white border-t">
            <div className="flex flex-col">
              ...
              <button
                onClick={() => { setActiveTab('accommodations'); setMobileMenuOpen(false); }}
                className={`px-4 py-3 text-sm font-medium text-left flex items-center space-x-3 ${
                  activeTab === 'accommodations'
                    ? 'text-white'
                    : 'bg-white text-gray-600 hover:bg-gray-50'
                }`}
                style={{ backgroundColor: activeTab === 'accommodations' ? '#5C2D91' : undefined }}
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <button
                onClick={() => { setActiveTab('driver-tracking'); setMobileMenuOpen(false); }}
                className={`px-4 py-3 text-sm font-medium text-left flex items-center space-x-3 ${
                  activeTab === 'driver-tracking'
                    ? 'text-white'
                    : 'bg-white text-gray-600 hover:bg-gray-50'
                }`}
                style={{ backgroundColor: activeTab === 'driver-tracking' ? '#5C2D91' : undefined }}
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <ul className="space-y-2">
            ...
            <li>
              <button 
                ...
                className={`w-full text-left px-4 py-2 rounded transition-colors flex items-center space-x-3 ${
                  activeTab === 'accommodations' ? 'text-white' : 'bg-gray-200 text-gray-800 hover:bg-gray-300'
                }`}
                style={{ backgroundColor: activeTab === 'accommodations' ? '#5C2D91' : undefined }}
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <li>
              <button 
                ...
                className={`w-full text-left px-4 py-2 rounded transition-colors flex items-center space-x-3 ${
                  activeTab === 'driver-tracking' ? 'text-white' : 'bg-gray-200 text-gray-800 hover:bg-gray-300'
                }`}
                style={{ backgroundColor: activeTab === 'driver-tracking' ? '#5C2D91' : undefined }}
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <main className="flex-1 p-4 lg:p-8">
              ...
              {user && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                  ...
                  <div className="bg-blue-50 p-4 sm:p-6 rounded-lg">
                    <h4 className="text-sm font-medium text-blue-700 mb-3">Account Status</h4>
                    <div className="space-y-2">
                      <p className="text-blue-600 text-sm sm:text-base">✓ Active User Account</p>
                      <p className="text-sm text-blue-500">You can access user-specific features here.</p>
                    </div>
                  </div>
                </div>
              )}
        ...
        {activeTab === 'accommodations' && (
          <div className="space-y-6">
            <AccommodationForm onSuccess={handleAccommodationSuccess} />
            <div className="bg-white rounded-lg shadow-md">
              <div className="px-6 py-4 border-b">
                <h3 className="text-lg font-semibold text-gray-900">My Accommodation Requests</h3>
              </div>
              <div className="p-6">
